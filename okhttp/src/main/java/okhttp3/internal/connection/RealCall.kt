...
/**
 * OKHttp应用层和网络层之间的桥梁。
 * 这个类对外暴露了一些高层次的应用层属性: connections, requests, responses, and streams.这些属性都是针对应用层而言的，也就是HTTP协议而言。
 *
 * RealCall是可以被异步取消的。这个操作有着最小的影响半径。[cancel]
 * 在HTTP/2中，这个取消会导致当前所在的流也会被关闭。
 * 如果是还在TLS握手过程中的话，会导致整个连接都被关闭。
 */
class RealCall(
